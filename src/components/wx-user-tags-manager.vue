<template>
  <a-modal title="公众号用户标签管理" :maskClosable="false" :visible.sync="dialogVisible" @cancel="dialogVisible = false">
    <a-spin size="large" :spinning="submitting" :indicator="indicator">
      <div class="panel  flex flex-wrap">
        <a-tag v-for="tag in wxUserTags" closable @click="editTag(tag.id,tag.name)" @close="deleteTag(tag.id)" :disable-transitions="false" :key="tag.id">
          {{tag.id}} {{tag.name}}
        </a-tag>
        <a-input class="input-new-tag" v-if="inputVisible" placeholder="回车确认" v-model="inputValue" ref="saveTagInput" size="small" @keyup.enter.native="addTag">
        </a-input>
