<template>
  <div class="mod-menu">
    ...
    <a-spin size="large" :spinning="dataListLoading" :indicator="indicator">
      <div>
        <div class="card" v-for="item in dataList" :key="item.mediaId" @click="onSelect(item)">
          <img v-if="fileType=='image'" class="card-image" :src="item.url" />
    ...
    <a-pagination @change="currentChangeHandle" :current="pageIndex" :pageSizeOptions="['20']" :pageSize="20" :total="totalCount">
    </a-pagination>
    <!-- 弹窗, 新增 / 修改 -->
...
export default {
  ...
  methods: {
    ...
    // 当前页
    currentChangeHandle(page, pageSize) {
      this.pageIndex = page
      this.getDataList()
    },
    onCopySuccess() {
